...
public class FifthRxtxCommunicationClient implements Runnable {
    ...
    public FifthRxtxCommunicationClient(BidirectionalService bidirectionalService, TelemCommunicationConfig telemCommunicationConfig) {
    ...
    public void run() {
        ...
        bootstrap.handler(new ChannelInitializer<RxtxChannel>() {
            ...
            protected void initChannel(RxtxChannel rxtxChannel) throws Exception {
                ...
                rxtxChannel.config().setBaudrate(115200)
                ...
                pipeline.addLast(new FifthRxtxConnectResponseHandler(bidirectionalService));
            }
        });

        Channel channel = null;
        try {
            ChannelFuture f = bootstrap.connect(new RxtxDeviceAddress(telemCommunicationConfig.getFifthRxtx())).sync();
            channel = f.channel();
