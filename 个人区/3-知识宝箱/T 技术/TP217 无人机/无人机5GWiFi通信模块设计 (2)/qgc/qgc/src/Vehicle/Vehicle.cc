...
void Vehicle::_initializeCsv()
{
    ...
    // 创建txt文件的完整路径
    QString logFilePath = logsDirPath + "/" + currentTime + ".txt";
    // 创建并打开文件

    if(m_txtFile==nullptr)
    m_txtFile= new QFile(logFilePath,this);
    if (m_txtFile->open(QIODevice::WriteOnly | QIODevice::Text)) {
        // logFile.write("This is a log file created at " + currentTime.toUtf8());
        // logFile.close();
        qDebug() << "Log file created:" << logFilePath;
    } else {
...
void Vehicle::_writeCsvLine()
{
   ...
   eee << www.join(",") << "\n";
      ...
      m_txtFile->flush();
      qDebug()<<www;
    // www << QString::number(latitude(),'f',7) ;
    // www << QString::number(longitude(),'f',7) ;
    // www << QString::number(_distanceSensorFactGroup.rotationPitch270()->rawValue().toFloat(),'f',2) ;

    // www << QString::number(_altitudeAMSLFact.rawValue().toFloat(),'f',2) ;
