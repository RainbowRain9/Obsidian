...
void MultiVehicleManager::sendVehicleInfosToServer()
{
    if(!activeVehicleAvailable())
    {
        return;
    }
    vehicleInfoPublish info;
    info.lat= _activeVehicle->latitude()*1e7;
    info.lon = _activeVehicle->longitude()*1e7;
    info.altitude=_activeVehicle->altitudeAMSL()->rawValue().toFloat()*1000;
    info.height=_activeVehicle->altitudeRelative()->rawValue().toFloat()*1000;
    info.id=_activeVehicle->id();

    QByteArray arr= QByteArray((const char*)&info,sizeof(vehicleInfoPublish));
    m_udp->writeDatagram(arr,QHostAddress("127.0.0.1"),40001);
}

void MultiVehicleManager::_handleCarInfo(mavlink_message_t &message)
{
    mavlink_efi_status_t status;
    mavlink_msg_efi_status_decode(&message, &status);


    bool carState=((int(status.rpm)) >> 0) & 1;
    bool lidaState=((int(status.rpm)) >> 1) & 1;
    bool podState=((int(status.rpm)) >> 0) & 1;
    bool mode=((int(status.rpm)) >> 1) & 1;

    setCarState(carState);
    setLidaState(lidaState);
    setPodState(podState);
    setCarMode(mode);

    setJiaoju(status.fuel_consumed);


    int flag= int(status.fuel_flow);
    if(flag ==1)
    {
        if((int)m_bizhangFaild==0)
        {
            //bizhang shibai
        }
    }else
    ...
    setDistance(status.engine_load);
    setSpeed(status.throttle_out);

    int tip = int(status.spark_dwell_time);
    if(tip==1)
    {
        if((int)m_tip==0)
        {
    ...
    qDebug()<<"wwwwwwwwww  "<<status.ecu_index;
}
...
float MultiVehicleManager::speed() const
{
    return m_speed;
}

void MultiVehicleManager::setSpeed(float newSpeed)
{
...
float MultiVehicleManager::distance() const
{
    return m_distance;
}

void MultiVehicleManager::setDistance(float newDistance)
{
...
float MultiVehicleManager::jiaoju() const
{
    return m_jiaoju;
}

void MultiVehicleManager::setJiaoju(float newJiaoju)
{
    if (qFuzzyCompare(m_jiaoju, newJiaoju))
        return;
    m_jiaoju = newJiaoju;
    emit jiaojuChanged();
}

bool MultiVehicleManager::carMode() const
{
    return m_carMode;
}

void MultiVehicleManager::setCarMode(bool newCarMode)
{
    if (m_carMode == newCarMode)
        return;
    m_carMode = newCarMode;
    emit carModeChanged();
}

bool MultiVehicleManager::podState() const
{
    return m_podState;
}

void MultiVehicleManager::setPodState(bool newPodState)
{
    if (m_podState == newPodState)
        return;
    m_podState = newPodState;
    emit podStateChanged();
}

bool MultiVehicleManager::lidaState() const
{
    return m_lidaState;
}

void MultiVehicleManager::setLidaState(bool newLidaState)
{
    if (m_lidaState == newLidaState)
        return;
    m_lidaState = newLidaState;
    emit lidaStateChanged();
}

bool MultiVehicleManager::carState() const
{
    return m_carState;
}

void MultiVehicleManager::setCarState(bool newCarState)
{
    if (m_carState == newCarState)
        return;
    m_carState = newCarState;
    emit carStateChanged();
}

float MultiVehicleManager::vol() const
{
    return m_vol;
}

void MultiVehicleManager::setVol(float newVol)
{
    if (qFuzzyCompare(m_vol, newVol))
        return;
    m_vol = newVol;
    emit volChanged();
}
