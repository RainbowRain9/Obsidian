...
FlightMap {
    ...
    property var startPoint :0  //测距开始坐标点
    property var endPoint: 0  //结束点

    //中间点
    property var middle: QtPositioning.coordinate((startPoint.latitude+endPoint.latitude)/2,(startPoint.longitude+endPoint.longitude)/2)

    //是否测距开关
    property bool m_measure : QGroundControl.measure
    ...
    function calculateDistance(start, end) {
        if (start && end) {
            var startCoord = QtPositioning.coordinate(start.latitude, start.longitude)
            var endCoord = QtPositioning.coordinate(end.latitude, end.longitude)
        ...
        id: circle1
                    ...
                    sourceItem: Rectangle {
                        ...
                        QGCLabel
                        {
                           ...
                           text: startPoint.toLocaleString() //startPoint.toString()
                           color:"green"
                        }
                    }
                }
    MapQuickItem {
        id: circle2
                    ...
                    sourceItem: Rectangle {
                        ...
                        QGCLabel
                        {
                           ...
                           //anchors.left: parent.left
                           anchors.horizontalCenter: parent.horizontalCenter
                           text: endPoint.toLocaleString() //startPoint.toString()
                           color:"green"
                        }
                    }
                }
       ...
       MapQuickItem {
   //                    anchorPoint.x: item.width / 2
   //                    anchorPoint.y: item.height / 2
                       coordinate: middle
                       sourceItem: Text {
                           text: calculateDistance(startPoint, endPoint).toFixed(3) + " m"
                           font.pixelSize: 25
                           color: "red"
                       }
                   }
