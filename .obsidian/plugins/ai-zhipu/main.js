...
var isDashDashDash = (line) => line.trim().startsWith("---") && line.trim().split("").every((el) => el === "-") && line.indexOf("-") < 3;
var getTemplates = (fileContent) => {
  const lines = fileContent.split("\n").map((el, index) => {
    return { content: el, index };
  });
  const dividers = lines.filter((line) => isDashDashDash(line.content));
...
var parseTemplate = (lines) => {
  ...
  const defaultParams = { model: Glm4 };
...
var AIZhipuPlugin = class extends import_obsidian4.Plugin {
  ...
  findCurrentBlock(editor) {
    ...
    if (isUserMarkStart(lines[start]) && isUserMarkEnd(lines[end]))
      return { type: "user", content, start, end };
    if (isAssistantMarkStart(lines[start]) && isAssistantMarkEnd(lines[end]))
      return { type: "assistant", content, start, end };
    return null;
  }
  findAboveComment(editor) {
  }
  findNextEmptyAssistBlock(editor, from) {
    const fileText = editor.getValue();
    const lines = fileText.split("\n");
    let l = from;
    let start = -1;
  ...
  async generateText(editor, template) {
    var _a3, _b2;
